...
**weave** is a promptable [SAM3](https://github.com/facebookresearch/sam3) finetuned for histopathology: instead of a fixed cell taxonomy it segments whatever `--patch_seg_prompt` names, at any resolution (it resizes internally, like a VLM; its native input is **1008×1008**, so `--patch_size 1008` avoids an extra resize). It outputs a **semantic region map** by default — the per-tile detections are dissolved into contiguous same-class regions (pass `--patch_seg_no_dissolve` for raw per-instance masks). Output is keyed **per prompt** (`seg_weave_<prompt>/`), so several prompts coexist on one `--job_dir`. weave runs on a **single GPU** (`--gpus 0`; for another GPU use `CUDA_VISIBLE_DEVICES=<n>`).
...
**Step 5: Asl ROIs with a vision-language model (VLM):** Ask a free-text question about tissue regions with [Patho-R1](https://huggingface.co/WenchuanZhang/Patho-R1-7B). 
 ...
 - **Batch** — ask the same question of every tissue patch. Consumes the coords from Step 2 (like cell segmentation):
   ```bash
   python run_batch_of_slides.py --task vlm --wsi_dir ./wsis --job_dir ./trident_processed \
...
> - VLM generation is **autoregressive and slow** compared with the feed-forward encoders, and `--task vlm` sweeps *every* tissue patch — prefer a tight coords set or a coarser field of view (fewer patches). It is **not** part of `--task all`. Lower `--vlm_batch_size` (default 4) if you run out of GPU memory; ~16 GB is enough for the 7B in bf16 (~8 GB for the 3B).
