...
class BasePatchEncoder(torch.nn.Module):
    ...
    def __init__(
        self, weights_path: Optional[str] = None, **build_kwargs: Dict[str, Any]
    ):
        """
...
class HibouLInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        if weights_path:
            raise NotImplementedError(
                "Hibou-Large doesn't support local model loading. PR welcome!"
            )
        else:
            self.ensure_has_internet(self.enc_name)
            try:
...
class Conchv15InferenceEncoder(BasePatchEncoder):
    ...
    def _build(self, img_size=448):
        from trident.patch_encoder_models.model_zoo.conchv1_5.conchv1_5 import (
...
class OpenMidnightInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        try:
            ...
            traceback.print_exc()
            raise Exception(
                "Failed to initialize DINOv2 ViT-G/14 backbone for OpenMidnight."
            )
...
class GPFMInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        ...
        if not weights_path:
            ...
            try:
                weights_path = hf_hub_download(
                    repo_id="majiabo/GPFM", filename="GPFM.pth"
                )
            except Exception:
                traceback.print_exc()
...
class GenBioPathFMInferenceEncoder(BasePatchEncoder):
    ...
    def _build(self):
        from huggingface_hub import hf_hub_download
        from torchvision.transforms import InterpolationMode
        from trident.patch_encoder_models.model_zoo.genbio_pathfm.genbio_pathfm import (
        ...
        if weights_path:
            try:
                ...
                raise Exception(
                    ...
                    "You can download the required model files from: https://huggingface.co/wisdomik/QuiltNet-B-16."
                )
        else:
            self.ensure_has_internet(self.enc_name)
            try:
                ...
                traceback.print_exc()
                raise Exception("Failed to download QuiltNet-B-16 model.")
